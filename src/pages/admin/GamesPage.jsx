        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {games.map((game) => {
            ...
            return (
              <article
                key={game.id}
                className="group bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:border-brand-200 transition-all duration-200 overflow-hidden dark:border-slate-700 dark:bg-slate-900 dark:hover:border-brand-500/50"
              >
                ...
                <div className="flex items-center gap-3 px-5 pt-5 pb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-semibold text-navy-900 truncate group-hover:text-brand-600 transition-colors dark:text-white dark:group-hover:text-brand-300">
                      {game.name}
                    </h3>
                    <p className="text-xs text-gray-400 truncate dark:text-slate-500">{typeName}</p>
                  </div>
                </div>

                {/* Date row */}
                <div className="px-5 pb-3 flex items-center gap-4 text-xs text-gray-500 dark:text-slate-400">
                  <span className="flex items-center gap-1">
                    ...
                    <span className="flex flex-col leading-tight">
                      ...
                      <span className="text-gray-400 dark:text-slate-500">{formatDate(game.start_at).time}</span>
                    </span>
                  </span>
                  <span className="text-gray-300 dark:text-slate-600">→</span>
                  <span className="flex flex-col leading-tight">
                    <span>{formatDate(game.end_at).date}</span>
                    <span className="text-gray-400 dark:text-slate-500">{formatDate(game.end_at).time}</span>
                  </span>
                </div>

                {/* Card actions */}
                <div className="border-t border-gray-100 px-5 py-3 flex items-center gap-2 dark:border-slate-700">
                  <Link
