...
export default function HomePage() {
  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div className="animate-fade-up">
              ...
              <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-navy-900 leading-[1.1] dark:text-white">
                {t('home.headline')}
              </h1>
              <p className="mt-6 text-lg sm:text-xl text-navy-600 leading-relaxed max-w-xl dark:text-slate-300">
                {t('home.lede')}
              </p>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {isAuthenticated ? (
                  <>
                    ...
                    <Link to="/team" className="inline-flex items-center rounded-full border-2 border-navy-200 px-6 py-3 text-sm font-semibold text-navy-700 hover:border-brand-300 hover:text-brand-600 transition-all dark:border-slate-600 dark:text-slate-200 dark:hover:border-brand-400 dark:hover:text-brand-300">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {['tagAllAges', 'tagScoutFriendly', 'tagTeamPlay', 'tagShortLong'].map((tag) => (
                  <span key={tag} className="inline-flex items-center gap-1.5 rounded-full bg-white/80 border border-warm-200 px-3 py-1.5 text-xs font-medium text-navy-600 dark:border-slate-700 dark:bg-slate-800/90 dark:text-slate-300">
            ...
            <aside className="animate-slide-in-right">
              <div className="rounded-2xl border border-warm-200 bg-white/90 backdrop-blur-sm shadow-xl shadow-navy-900/5 p-6 sm:p-8 dark:border-slate-700 dark:bg-slate-900/95 dark:shadow-black/30">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wider text-navy-400 dark:text-slate-500">{t('home.quickStart')}</p>
                    <h2 className="font-display text-xl font-bold text-navy-900 mt-1 dark:text-white">{t('home.teamLogin')}</h2>
                  </div>
                ...
                <p className="text-sm text-navy-500 mb-6 dark:text-slate-400">{t('home.panelLede')}</p>
      ...
      {/* Social proof bar */}
      <section className="border-y border-warm-200/60 bg-white/60 backdrop-blur-sm dark:border-slate-700/80 dark:bg-slate-900/60">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-wrap justify-center gap-8 sm:gap-12 text-center">
            <div>
              <p className="font-display text-2xl font-bold text-navy-900 dark:text-white">13+</p>
              <p className="text-xs text-navy-500 mt-1 dark:text-slate-400">{t('home.proofGames')}</p>
            </div>
            <div>
              <p className="font-display text-2xl font-bold text-navy-900 dark:text-white">21</p>
              <p className="text-xs text-navy-500 mt-1 dark:text-slate-400">{t('home.proofLanguages')}</p>
            </div>
            <div>
              <p className="font-display text-2xl font-bold text-navy-900 dark:text-white">{t('home.proofRealtimeValue')}</p>
              <p className="text-xs text-navy-500 mt-1 dark:text-slate-400">{t('home.proofRealtime')}</p>
            </div>
            <div>
              <p className="font-display text-2xl font-bold text-brand-600">{t('home.proofFreeValue')}</p>
              <p className="text-xs text-navy-500 mt-1 dark:text-slate-400">{t('home.proofFree')}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 sm:py-28 bg-white dark:bg-slate-950">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-navy-900 dark:text-white">{t('home.featuresTitle')}</h2>
            <p className="mt-4 text-lg text-navy-500 dark:text-slate-400">{t('home.featuresSubtitle')}</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <article key={f.title} className="group rounded-2xl border border-warm-200 bg-warm-50/50 p-8 hover:border-brand-200 hover:shadow-lg hover:shadow-brand-500/5 transition-all duration-300 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-brand-500/50">
                <span className="text-3xl mb-4 block">{f.icon}</span>
                <h3 className="font-display text-lg font-bold text-navy-900 mb-2 dark:text-white">{f.title}</h3>
                <p className="text-sm text-navy-600 leading-relaxed dark:text-slate-300">{f.text}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 sm:py-28 bg-gradient-to-b from-warm-50 to-white dark:from-slate-900 dark:to-slate-950">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-navy-900 dark:text-white">{t('home.stepsTitle')}</h2>
            <p className="mt-4 text-lg text-navy-500 dark:text-slate-400">{t('home.stepsSubtitle')}</p>
          </div>
          <div className="grid sm:grid-cols-3 gap-8 lg:gap-12">
            {[1, 2, 3].map((n) => (
              <div key={n} className="relative text-center group">
                ...
                <h4 className="font-display text-lg font-bold text-navy-900 mb-2 dark:text-white">{t(`home.step${n}Title`)}</h4>
                <p className="text-sm text-navy-600 leading-relaxed max-w-xs mx-auto dark:text-slate-300">{t(`home.step${n}Text`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 sm:py-28 dark:bg-slate-950">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="font-display text-3xl sm:text-4xl font-bold text-navy-900 dark:text-white">{t('home.ctaBottomTitle')}</h2>
